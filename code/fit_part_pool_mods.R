...
zib_mod <- bf(
  ...
  CI_pred.sc + (1|trap) + (1|year) + (1|SP4) + (1|OLRE) + (1|quadrat),
  zi ~ CI_pred.sc + (1|trap) + (1|year)+ (1|SP4) + (1|OLRE) + (1|quadrat),
  family = zero_inflated_binomial(logit))


## Zero-One inflated beta
zoib_mod <- bf(
  proportion_abscised ~ CI_pred.sc + (1|trap) + (1|year) + (1|SP4) + (1|quadrat), 
  phi ~ CI_pred.sc + (1|trap) + (1|year) + (1|SP4) + (1|quadrat), 
  zoi ~ CI_pred.sc + (1|trap) + (1|year) + (1|SP4) + (1|quadrat), 
  coi ~ CI_pred.sc + (1|trap) + (1|year) + (1|SP4) + (1|quadrat), 
...
# the models will run sequentially, and 4 cores per model in parallel.
...
all_models_quad <- run_my_models(data = testdat, name = "full_sp_quad")

#save on cluster
save(all_models_quad, file = "/home/users/ft840275/SpatialPatterns/output/mods_full_quad.RData")